...
#				self.activity)
	...
	def fingerprint_combos(self, pref_fingerprint):
		dead = [strn for strn in pref_fingerprint if pref_fingerprint[strn] == 1]
		alive = [strn for strn in pref_fingerprint if pref_fingerprint[strn] == 0]
		
		combos = []
		
		for binary in itertools.product([0, 1], repeat=len(dead)):
			synprint = dict([(al, 0) for al in alive])
			for strn, val in zip(dead, binary):
				synprint[strn] = val
			combos.append(synprint)
			
		return combos
		
	def tally_fingerprints(self):
	
		self.tally = dict()
	
		for pref, fprint_combos in self.fingerprints.items():
			for combo in fprint_combos:
				imut_combo = frozenset(combo.items())
				self.tally.setdefault(imut_combo, [])
				self.tally[imut_combo].append(pref)		
	
		best = 0
		for fprint, prefs in self.tally.items():
			if len(prefs) > best and not sum(zip(*fprint)[1]) == 0:
				best = len(prefs)
			if len(prefs) < 1:
				del  self.tally[fprint]
		
		self.inverse_count = 1.0 / len(self.tally)
		self.fraction_covered = float(best) / len(self.peaks)
		
	def subfingerprint(self, *pref_fingerprints):
		"""Function accepts a series of prefraction fingerprint dictionaries, and returns a dictionary that
		is the intersection of the fingerprints. For a strain, the subfingerprint value is the sum of the
		number of prefractions that hit (value of 1).
		"""
		subprint = dict()
		if all([d.keys() == pref_fingerprints[0].keys() for d in pref_fingerprints]):
			for strn_overlap, vals in [zip(*pair) for pair in zip(*[d.items() for d in pref_fingerprints])]:
				subprint[strn_overlap[0]] = numpy.mean(vals)
		
		self.activity = sum(map(int, subprint.values()))
		
		if all([val == 0 for val in subprint.values()]):
			self.cluster_score = 0
		else:
			self.cluster_score = numpy.mean([v for v in subprint.values()if not v == 0])
...
class heatmap:
	...
	def __init__(self, *peaks):
		...
		if not len(peaks):
			return
		for bask in self.basketer(10, 0.4, 0.5, *peaks):
	...
	def export_nxn(self, idrun_list=[]):
		...
		idruns = self.idruns()
		if idrun_list:
			idruns = [id for id in idrun_list]
		baskets = sorted(self.baskets(), key=lambda bask: bask.m)
		
		nxn = numpy.zeros((len(idruns), len(baskets)))
		for iind, idrun in enumerate(idruns):
			for bind, bask in enumerate(baskets):
				if idrun.name in self.map:
					if bask.name in self.map[idrun.name].peaks:
						nxn[(iind, bind)] = bask.rt
		return nxn, [str(id) for id in idruns], [str(bask) for bask in baskets]
	...
	def plot_inverse_count(self, name):
		basks = self.baskets()
		x = range(len(basks))
		y = sorted([bask.inverse_count for bask in basks])
		f, (ax1, ax2) = plt.subplots(1, 2, sharey=False)
		ax1.scatter(x, y)
		ax2.hist(y, 30)
		plt.savefig(name + "_inverse_count.pdf")
		plt.close()
		
	def plot_fraction_covered(self, name):
		basks = self.baskets()
		x = range(len(basks))
		y = sorted([bask.fraction_covered for bask in basks])
		f, (ax1, ax2) = plt.subplots(1, 2, sharey=False)
		ax1.scatter(x, y)
		ax2.hist(y, 30)
		plt.savefig(name + "_fraction_covered.pdf")
		plt.close()
